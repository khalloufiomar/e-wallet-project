...
export class InvoiceDetailsComponent implements OnInit {
  ...
  private initConfig(): void {
    ...
    this.payPalConfig = {
      currency: 'USD',
      clientId: 'AdkAgKhLGFuGf9K3hiBKvdat45f4wBbUuJU7Zm26Sx_C4PsrjMtzNrSCw4HWaJS-w2oHwlGLmWcLpRIn',
      createOrderOnServer: () => {
        return fetch(`http://localhost:8069/fund_wallet/${this.invoiceId}/pay_invoice`, {
          method: 'POST',
          headers: {
      ...
      onApprove: async (data, actions) => {
        ...
        try {
          const response = await fetch(`http://localhost:8069/api/orders/${data.orderID}/capture`, {
            method: "POST",
            headers: {
              'Content-Type': 'application/json',
              'Accept': 'application/json'
